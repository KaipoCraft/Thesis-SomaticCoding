...
class GestureInterpreter:
    def __init__(self):
        self.gesture_state_observers = []

    def attach_observer(self, observer):
        self.gesture_state_observers.append(observer)
    def detach_observer(self, observer):
        self.gesture_state_observers.remove(observer)
    def notify_observers(self, gesture):
        for observer in self.gesture_state_observers:
...
class ClockwiseCirle(GestureStates):
...
class CounterClockwiseCirle(GestureStates):
    def __init__(self):
        super().__init__()
        self.shape = ["→", "↑", "←", "↓"]
